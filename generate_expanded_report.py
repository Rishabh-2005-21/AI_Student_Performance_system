...
def generate_expanded_report():
    ...
    print("Building master report with expanded technical code & details...")
    ...
    p_rep = doc.add_paragraph(); p_rep.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_rep.paragraph_format.space_before = Pt(12); p_rep.paragraph_format.space_after = Pt(4)
    ...
    p_deg = doc.add_paragraph(); p_deg.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    p_title_lbl = doc.add_paragraph(); p_title_lbl.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    p_proj_title = doc.add_paragraph(); p_proj_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    p_foot = doc.add_paragraph(); p_foot.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    create_custom_table(col_widths=[5.0, 1.5], headers=["Section", "Page No."], data=[
        ["Certificate", "i"], ["Certificate of Approval", "ii"], ["Student Declaration", "iii"], ["Acknowledgement", "iv"], ["Abstract", "v"], ["List of Figures", "vi"], ["List of Tables", "vii"], ["Project Progress Report", "viii"],
        ["Chapter 1: Introduction", "1"], ["Chapter 2: Literature Review / Background Study", "8"], ["Chapter 3: Requirement Analysis and Planning", "20"], ["Chapter 4: System Design and Methodology", "31"], ["Chapter 5: Implementation", "48"], ["Chapter 6: Testing, Results, and Discussion", "65"], ["Chapter 7: Conclusion and Future Scope", "75"],
        ["References", "79"], ["Appendix A: Installation and User Manual", "82"], ["Appendix B: Team Contribution Matrix", "86"], ["Appendix C: Academic Integrity and Tool Disclosure", "87"]
    ])
    ...
    create_custom_table(col_widths=[1.5, 4.0, 1.0], headers=["Figure No.", "Figure Title", "Page No."], data=[
    ...
    create_custom_table(col_widths=[1.5, 4.0, 1.0], headers=["Table No.", "Table Title", "Page No."], data=[
    ...
    add_callout("Motivation Summary: The system transforms academic assessment from a passive, delayed grading task into an active, intelligent, real-time diagnostic engine capable of revealing hidden misconceptions and overconfidence.", title="CORE MOTIVATION")
    ...
    add_body_p("This report is structured into seven comprehensive chapters and three technical appendices detailing every facet of system architecture, implementation, and empirical validation.")
    add_page_break()
...
20. RETURN next_question""", caption="Listing 4.2: Algorithmic Pseudocode for Dynamic CAT Item Selection")
    ...
    add_body_p("Below are full code listings from core system backend modules:")

    add_code_block("""# File: server/app.py
...
def create_app() -> Flask:
    ...
    @app.get("/")
    def root(): return send_from_directory(CLIENT_DIR, "index.html")

    @app.get("/<path:path>")
    def catch_all(path): return send_from_directory(CLIENT_DIR, "index.html")

    return app

app = create_app()
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True)""", caption="Listing 5.1: Flask Entry Point Server app.py")

    add_code_block("""# File: server/ai_engine/question_generator.py
import json, os, re, uuid
from typing import List, Dict

GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "")

def generate_questions_from_text(text: str, subject: str, question_types: List[str], difficulties: List[str], count: int) -> List[Dict]:
    if GEMINI_API_KEY:
        try:
            import google.generativeai as genai
            genai.configure(api_key=GEMINI_API_KEY)
            model = genai.GenerativeModel("gemini-1.5-flash")
            prompt = f"Generate {count} questions for subject {subject} on topics in: {text[:4000]}"
    ...
    for i in range(count):
        questions.append({
            "id": str(uuid.uuid4()),
            "question": f"Sample Question {i+1} on {subject}?",
            ...
            "correct_answer": "Option A" if "mcq" in question_types else "sample",
            "topic": subject
        })
    return questions""", caption="Listing 5.2: Gemini Question Bank Generator in question_generator.py")
...
def evaluate_fill_blank(student_answer: str, correct_answer: str) -> tuple[bool, float]:
    ratio = SequenceMatcher(None, student_answer.strip().lower(), correct_answer.strip().lower()).ratio()
    return ratio >= 0.8, round(ratio, 2)

def evaluate_integer(student_answer: str, correct_answer: str) -> tuple[bool, float]:
    try:
        is_correct = int(student_answer.strip()) == int(correct_answer.strip())
        return is_correct, 1.0 if is_correct else 0.0
    except:
        return False, 0.0
...
def evaluate_subjective(student_answer: str, model_answer: str) -> tuple[bool, float]:
    ...
    if not s or not m: return False, 0.0
    ...
    if api_key:
        try:
            ...
            res = openai.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "system", "content": "Output ONLY similarity score between 0.0 and 1.0 based on conceptual accuracy."},
                          {"role": "user", "content": f"Model: {m}\\nStudent: {s}"}],
                temperature=0.0
            )
...
def calculate_weighted_score(is_correct: bool, confidence: str, difficulty: str) -> dict:
    ...
    acc_score = 60 if is_correct else 0
    if is_correct and conf == "high": conf_score = 20
    elif is_correct and conf == "medium": conf_score = 12
    elif is_correct and conf == "low": conf_score = 5
    elif not is_correct and conf == "high": conf_score = -15
    else: conf_score = 0
    diff_score = DIFFICULTY_TO_WEIGHT.get(diff, 5)
    return {"accuracy": acc_score, "confidence": conf_score, "difficulty": diff_score, "total": acc_score + conf_score + diff_score}""", caption="Listing 5.3: Answer Evaluation & Metacognitive Matrix Engine in evaluator.py")
    ...
    file_name = "AI_Student_Performance_System_Minor_Project_Report.docx"
    doc.save(file_name)
    print(f"Master report saved successfully as {file_name}")
